...
check_acme_execute_http01_helper_contract() {
  local run_id="fixture-tls-acme-http01"
  local run_root="$WORKDIR/runs/$run_id"
  local artifact_root="$WORKDIR/artifacts/$run_id"
  local execute_blocker="pre-client blocked: 独立 execute helper 当前只落 non-placeholder companion contract，还不会调用 ACME client 或真实签发证书"
  local execute_next_step="当前 cut 1 只把 execute 语义独立落成 non-placeholder companion result；后续若要真实签发，仍需补 ACME client invocation / challenge fulfillment / certificate artifact write / deployment boundary control。"
  ...
  bash "$ROOT_DIR/acme-execute-http01.sh" \
    --issue-result-json "$artifact_root/ISSUE-RESULT.json" >/dev/null

  assert_contract_file "$artifact_root/ISSUE-RESULT.json" "issue-result"
  assert_json_value_equals "$artifact_root/ISSUE-RESULT.json" "contract_scope" "planning-evidence-only" "$run_id independent execute input issue result contract scope stays planning only"
  assert_json_value_equals "$artifact_root/ISSUE-RESULT.json" "mode" "dry-run" "$run_id independent execute input issue result mode stays dry-run"
  assert_json_value_equals "$artifact_root/ISSUE-RESULT.json" "reserved_execute_result.artifact_json" "ACME-ISSUANCE-RESULT.json" "$run_id independent execute input issue result reserved execute artifact json"
  assert_json_path_missing "$artifact_root/ISSUE-RESULT.json" "intent.result_role" "$run_id independent execute input issue result must not drift into execute intent payload"

  assert_contract_file "$artifact_root/ACME-ISSUANCE-RESULT.json" "acme-issuance-result"
  assert_json_paths "$artifact_root/ACME-ISSUANCE-RESULT.json" "$run_id independent execute stable paths" \
    mode final_status \
    ...
    execution.attempted_hosts execution.challenge_strategy execution.client_adapter execution.materialization execution.fulfilled_challenge_strategy execution.client_invoked execution.issued_certificate \
    artifacts.materialization artifacts.cert_path artifacts.key_path artifacts.fullchain_path \
  ...
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "mode" "execute" "$run_id independent execute mode"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "final_status" "blocked" "$run_id independent execute final status"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "placeholder.is_placeholder" "False" "$run_id independent execute placeholder marker false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "placeholder.placeholder_kind" "future-real-execute" "$run_id independent execute placeholder kind"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "placeholder.review_required" "False" "$run_id independent execute review_required false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "placeholder.source_of_truth" "independent-pre-client-blocked-helper" "$run_id independent execute source_of_truth"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "planning_reference.issue_result_json" "ISSUE-RESULT.json" "$run_id independent execute planning reference json"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "planning_reference.issue_result_markdown" "ISSUE-RESULT.md" "$run_id independent execute planning reference markdown"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "planning_reference.contract_scope" "planning-evidence-only" "$run_id independent execute planning reference scope"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "intent.result_role" "real-execute-attempt" "$run_id independent execute intent result role"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "intent.real_execution_performed" "False" "$run_id independent execute real_execution_performed false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "request.challenge_mode" "standalone" "$run_id independent execute challenge mode"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "request.acme_client" "manual" "$run_id independent execute acme client"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "pending_execution_plan.planned_challenge_mode" "standalone" "$run_id independent execute planned challenge mode"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "pending_execution_plan.planned_acme_client" "manual" "$run_id independent execute planned acme client"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "pending_execution_plan.planned_acme_directory" "staging" "$run_id independent execute planned acme directory"
  assert_json_array_contains "$artifact_root/ACME-ISSUANCE-RESULT.json" "pending_execution_plan.planned_target_hosts" "github.example.com" "$run_id independent execute planned target hosts"
  assert_json_array_contains "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.attempted_hosts" "github.example.com" "$run_id independent execute attempted hosts"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.challenge_strategy" "standalone" "$run_id independent execute challenge strategy slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.client_adapter" "manual" "$run_id independent execute client adapter slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.materialization" "deferred-until-real-execute" "$run_id independent execute materialization slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.fulfilled_challenge_strategy" "pre-client-blocked" "$run_id independent execute fulfilled challenge strategy"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.client_invoked" "False" "$run_id independent execute client_invoked false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.issued_certificate" "False" "$run_id independent execute issued_certificate false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "artifacts.materialization" "not-materialized" "$run_id independent execute artifacts materialization"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "deployment_boundary.writes_live_tls_paths" "False" "$run_id independent execute writes_live_tls_paths false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "deployment_boundary.modifies_live_nginx" "False" "$run_id independent execute modifies_live_nginx false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "deployment_boundary.reloads_nginx" "False" "$run_id independent execute reloads_nginx false"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "recovery.recoverable" "True" "$run_id independent execute recovery recoverable"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "recovery.blocker_summary" "$execute_blocker" "$run_id independent execute blocker summary"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "next_step" "$execute_next_step" "$run_id independent execute next step"

  assert_json_value_equals "$run_root/state.json" "artifacts.issue_result_json" "$artifact_root/ISSUE-RESULT.json" "$run_id independent execute state issue result snapshot"
  assert_json_value_equals "$run_root/state.json" "artifacts.acme_issuance_result_json" "$artifact_root/ACME-ISSUANCE-RESULT.json" "$run_id independent execute state acme issuance snapshot"
  assert_json_value_equals "$artifact_root/INSTALLER-SUMMARY.generated.json" "artifacts.acme_issuance_result_json" "$artifact_root/ACME-ISSUANCE-RESULT.json" "$run_id independent execute generated summary acme issuance snapshot"
  assert_json_value_equals "$artifact_root/INSTALLER-SUMMARY.json" "artifacts.acme_issuance_result_json" "$artifact_root/ACME-ISSUANCE-RESULT.json" "$run_id independent execute summary acme issuance snapshot"

  assert_journal_event_path_equals_state_artifact "$run_id" "issue.result.recorded" "issue_result_json" "$run_id independent execute issue.result.recorded path points to issue_result_json"
  assert_journal_event_path_equals_state_artifact "$run_id" "acme_issuance.result.recorded" "acme_issuance_result_json" "$run_id independent execute acme_issuance.result.recorded path points to acme_issuance_result_json"
}
...
check_acme_issue_http01_helper_execute_contract() {
  ...
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.challenge_strategy" "standalone" "$run_id execute placeholder challenge strategy slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.client_adapter" "manual" "$run_id execute placeholder client adapter slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.materialization" "deferred-until-real-execute" "$run_id execute placeholder materialization slot"
  ...
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "artifacts.materialization" "not-materialized" "$run_id execute placeholder artifacts materialization"
...
check_fixture_tls_acme_real_execute_attempt_contract() {
  ...
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.challenge_strategy" "standalone" "$run_id execution challenge strategy slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.client_adapter" "manual" "$run_id execution client adapter slot"
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "execution.materialization" "deferred-until-real-execute" "$run_id execution materialization slot"
  ...
  assert_json_value_equals "$artifact_root/ACME-ISSUANCE-RESULT.json" "artifacts.materialization" "not-materialized" "$run_id artifacts materialization"
...
assert_contains "$install_help_output" "inspect-after-apply-attention / inspect-after-acme-placeholder / inspect-after-acme-real-execute-attempt / repair-review-first / post-repair-verification / post-rollback-inspection" "install help surfaces inspection-first resume strategy set"
...
assert_contains "$doctor_resumed_output" "- 允许动作：继续人工排查；必要时再次 repair；显式 dry-run。" "resumed doctor prints allowed actions for repair-review-first"
assert_contains "$doctor_resumed_output" "- 禁止动作：不要把未收口 repair run 当成普通 resume；不要跳过复查直接真实 apply。" "resumed doctor prints disallowed actions for repair-review-first"
...
assert_contains "$doctor_current_apply_attention_output" "[doctor] resume 预览" "current apply attention doctor prints resume preview section"
assert_contains "$doctor_current_apply_attention_output" "- 如果现在继续 --resume，预计策略：inspect-after-apply-attention" "current apply attention doctor previews inspect-after-apply-attention strategy"
assert_contains "$doctor_current_apply_attention_output" "- 允许动作：复用输入/产物；显式 --run-apply-dry-run；先人工复查 apply result / recovery。" "current apply attention doctor previews allowed actions"
assert_contains "$doctor_current_apply_attention_output" "- 禁止动作：不要把 --resume 当成真实 apply 重放；不要直接带 --execute-apply。" "current apply attention doctor previews disallowed actions"
...
doctor_acme_placeholder_output="$(state_doctor "fixture-tls-acme-http01")"
assert_contains "$doctor_acme_placeholder_output" "[doctor] resume 预览" "acme placeholder doctor prints resume preview section"
assert_contains "$doctor_acme_placeholder_output" "- 如果现在继续 --resume，预计策略：inspect-after-acme-placeholder" "acme placeholder doctor previews placeholder strategy"
assert_contains "$doctor_acme_placeholder_output" "- 允许动作：复用输入/产物；显式 --run-apply-dry-run；人工复查 ACME companion result / issue result / operator prerequisites。" "acme placeholder doctor previews allowed actions"
assert_contains "$doctor_acme_placeholder_output" "- 禁止动作：不要把 --resume 当成真实 ACME issue / 真实 apply 续跑；不要直接带 --execute-apply。" "acme placeholder doctor previews disallowed actions"
assert_contains "$doctor_acme_placeholder_output" "- path: $WORKDIR/artifacts/fixture-tls-acme-http01/ACME-ISSUANCE-RESULT.json" "acme placeholder doctor prints acme issuance path"
assert_contains "$doctor_acme_placeholder_output" "- mode: execute" "acme placeholder doctor prints execute mode"
...
assert_contains "$doctor_acme_placeholder_output" "- pending_execution_plan.planned_target_hosts: github.example.com, raw.example.com, gist.example.com, assets.example.com, archive.example.com, download.example.com" "acme placeholder doctor prints planned target hosts"
assert_contains "$doctor_acme_placeholder_output" "- pending_execution_plan.planned_challenge_mode: standalone" "acme placeholder doctor prints planned challenge mode"
assert_contains "$doctor_acme_placeholder_output" "- pending_execution_plan.planned_challenge_fulfillment: standalone" "acme placeholder doctor prints planned challenge fulfillment"
assert_contains "$doctor_acme_placeholder_output" "- pending_execution_plan.planned_acme_client: manual" "acme placeholder doctor prints planned acme client"
assert_contains "$doctor_acme_placeholder_output" "- pending_execution_plan.planned_acme_directory: staging" "acme placeholder doctor prints planned acme directory"
assert_contains "$doctor_acme_placeholder_output" "- execution.challenge_strategy: standalone" "acme placeholder doctor prints execution challenge strategy"
assert_contains "$doctor_acme_placeholder_output" "- execution.client_adapter: manual" "acme placeholder doctor prints execution client adapter"
assert_contains "$doctor_acme_placeholder_output" "- execution.materialization: deferred-until-real-execute" "acme placeholder doctor prints execution materialization"
assert_contains "$doctor_acme_placeholder_output" "- execution.fulfilled_challenge_strategy: not-executed" "acme placeholder doctor prints fulfilled challenge strategy"
assert_contains "$doctor_acme_placeholder_output" "- execution.client_invoked: False" "acme placeholder doctor prints client_invoked false"
assert_contains "$doctor_acme_placeholder_output" "- execution.issued_certificate: False" "acme placeholder doctor prints issued_certificate false"
assert_contains "$doctor_acme_placeholder_output" "- artifacts.materialization: not-materialized" "acme placeholder doctor prints artifacts materialization"
assert_contains "$doctor_acme_placeholder_output" "- deployment_boundary.writes_live_tls_paths: False" "acme placeholder doctor prints tls boundary false"
assert_contains "$doctor_acme_placeholder_output" "- deployment_boundary.modifies_live_nginx: False" "acme placeholder doctor prints nginx boundary false"
assert_contains "$doctor_acme_placeholder_output" "- deployment_boundary.reloads_nginx: False" "acme placeholder doctor prints reload boundary false"
assert_contains "$doctor_acme_placeholder_output" "- operator_prerequisites.pending: review_issue_result_before_execute, implement_real_execute_path, confirm_challenge_fulfillment_path, confirm_certificate_write_target, confirm_deployment_boundary" "acme placeholder doctor prints prerequisite summary"
...
assert_contains "$doctor_non_placeholder_acme_output" "[doctor] resume 预览" "future real execute blocked doctor prints resume preview section"
assert_contains "$doctor_non_placeholder_acme_output" "- 如果现在继续 --resume，预计策略：inspect-after-acme-real-execute-attempt" "future real execute blocked doctor previews dedicated strategy"
assert_contains "$doctor_non_placeholder_acme_output" "- 允许动作：复用输入/产物；显式 --run-apply-dry-run；人工复查 ACME / ISSUE result 与 operator prerequisites。" "future real execute blocked doctor previews allowed actions"
assert_contains "$doctor_non_placeholder_acme_output" "- 禁止动作：不要把 --resume 当成真实签发或真实 apply 的续跑入口；不要直接带 --execute-apply。" "future real execute blocked doctor previews disallowed actions"
...
assert_contains "$doctor_non_placeholder_acme_output" "- execution.challenge_strategy: standalone" "future real execute blocked doctor prints execution challenge strategy"
assert_contains "$doctor_non_placeholder_acme_output" "- execution.client_adapter: manual" "future real execute blocked doctor prints execution client adapter"
assert_contains "$doctor_non_placeholder_acme_output" "- execution.materialization: deferred-until-real-execute" "future real execute blocked doctor prints execution materialization"
assert_contains "$doctor_non_placeholder_acme_output" "- execution.fulfilled_challenge_strategy: standalone-synthetic-attempt" "future real execute blocked doctor prints fulfilled challenge strategy"
assert_contains "$doctor_non_placeholder_acme_output" "- execution.client_invoked: True" "future real execute blocked doctor prints client_invoked true"
assert_contains "$doctor_non_placeholder_acme_output" "- execution.issued_certificate: False" "future real execute blocked doctor prints issued_certificate false"
assert_contains "$doctor_non_placeholder_acme_output" "- artifacts.materialization: not-materialized" "future real execute blocked doctor prints artifacts materialization"
...
assert_contains "$doctor_post_rollback_output" "- 允许动作：复核 rollback 结果；显式 dry-run；必要时新开 run。" "post rollback doctor prints allowed actions"
assert_contains "$doctor_post_rollback_output" "- 禁止动作：不要把 rollback 后现场直接视为可继续 execute 的干净断点；不要直接带 --execute-apply。" "post rollback doctor prints disallowed actions"
...
set -e
assert_equals "$inspect_after_acme_execute_rc" "2" "inspect-after-acme placeholder resume rejects explicit execute apply"
...
assert_contains "$non_placeholder_blocked_resume_output" "本次 resume 策略：inspect-after-acme-real-execute-attempt" "future real execute blocked fixture surfaces dedicated strategy name"
...
assert_contains "$non_placeholder_blocked_execute_output" "当前 resume 策略 inspect-after-acme-real-execute-attempt 不允许直接执行真实 apply" "future real execute blocked fixture execute refusal keeps dedicated strategy-specific block"
assert_contains "$non_placeholder_blocked_execute_output" "这类 inspection-first 续接（包括 inspect-after-apply-attention / inspect-after-acme-placeholder / inspect-after-acme-real-execute-attempt / repair-review-first / post-repair-verification / post-rollback-inspection）必须先按 doctor / repair / rollback 结论完成复查。" "future real execute blocked fixture execute refusal explains expanded inspection-first family"
assert_contains "$non_placeholder_blocked_execute_output" "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前会进入 inspect-after-acme-real-execute-attempt / review-first 续接，应先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发 / 真实 apply 的续跑入口。" "future real execute blocked fixture execute refusal prints dedicated strategy warning"
restore_generated_runs
trap 'rm -rf "$WORKDIR"' EXIT
