...
Recovery helpers (first iteration):
  - --resume <run_id> : reload a previous run's captured inputs and re-enter from a safe boundary; when lineage/recovery indicates inspection-first (including inspect-after-apply-attention / inspect-after-acme-placeholder / inspect-after-acme-real-execute-attempt / repair-review-first / post-repair-verification / post-rollback-inspection), resume defaults to review-first continuation instead of real apply replay
...
resume_allows_execute() {
  if resume_requires_review_boundary "${1:-}"; then
    return 1
  fi
...
if [[ -n "$RESUME_RUN_ID" ]]; then
  ...
  if ! resume_allows_execute "$RESUME_STRATEGY"; then
    if [[ "$CLI_REQUEST_EXECUTE_APPLY" == "1" ]]; then
      if warning_message="$(resume_strategy_review_warning_message "$RESUME_STRATEGY" 2>/dev/null)"; then
        ui_warn "$warning_message"
        if [[ "$RESUME_STRATEGY" == inspect-after-acme-placeholder || "$RESUME_STRATEGY" == inspect-after-acme-real-execute-attempt ]]; then
        ...
        ui_warn "$(resume_source_acme_real_execute_reuse_warning_message "execute-refusal")"
        if [[ -n "$RESUME_SOURCE_ACME_NEXT_STEP" ]]; then
          ui_warn "ACME 建议：$RESUME_SOURCE_ACME_NEXT_STEP"
        fi
      fi
      ui_error "$(resume_strategy_execute_apply_refusal_message "$RESUME_STRATEGY")"
      exit 2
    fi
    EXECUTE_APPLY="0"
    RUN_NGINX_TEST_AFTER_EXECUTE="0"
...
if [[ "$INSTALLER_MODE" == "resume" ]]; then
  if resume_requires_review_boundary "$RESUME_STRATEGY"; then
    ui_info "当前以 resume 模式启动：这轮属于 review-first 续接；会优先复查可复用产物，不把继续真实 apply 当默认动作。"
  else
  ...
  if [[ "$RESUME_SOURCE_ROLLBACK_MODE" == "execute" && "$RESUME_SOURCE_ROLLBACK_FINAL_STATUS" == "ok" ]]; then
    ...
    ui_warn "$(resume_strategy_review_warning_message "inspect-after-apply-attention")"
    if [[ -n "$RESUME_SOURCE_APPLY_NEXT_STEP" ]]; then
      ui_warn "源运行建议：$RESUME_SOURCE_APPLY_NEXT_STEP"
    fi
  elif [[ "${RESUME_SOURCE_ACME_REVIEW_REQUIRED:-0}" == "1" ]]; then
    ui_warn "$(resume_strategy_review_warning_message "inspect-after-acme-placeholder")"
    if [[ -n "$RESUME_SOURCE_ACME_NEXT_STEP" ]]; then
      ui_warn "ACME 建议：$RESUME_SOURCE_ACME_NEXT_STEP"
    fi
  elif [[ "${RESUME_SOURCE_ACME_REAL_EXECUTE_ATTEMPT:-0}" == "1" ]]; then
    ui_warn "$(resume_source_acme_real_execute_reuse_warning_message)"
    if [[ -n "$RESUME_SOURCE_ACME_NEXT_STEP" ]]; then
      ui_warn "ACME 建议：$RESUME_SOURCE_ACME_NEXT_STEP"
    fi
  fi
