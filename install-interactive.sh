...
resume_allows_execute() {
  if resume_strategy_allows_execute "${1:-}"; then
  ...
  if ! resume_allows_execute "$RESUME_STRATEGY"; then
    if [[ "$CLI_REQUEST_EXECUTE_APPLY" == "1" ]]; then
      if [[ "$RESUME_STRATEGY" == "inspect-after-acme-placeholder" ]]; then
        ...
        ui_warn "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前虽然复用的是既有 apply-plan / generated-output 边界，但仍应先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发 / 真实 apply 的续跑入口。"
        if [[ -n "$RESUME_SOURCE_ACME_NEXT_STEP" ]]; then
          ui_warn "ACME 建议：$RESUME_SOURCE_ACME_NEXT_STEP"
        fi
      fi
...
if [[ "$INSTALLER_MODE" == "resume" ]]; then
  ...
  if [[ "$RESUME_SOURCE_ROLLBACK_MODE" == "execute" && "$RESUME_SOURCE_ROLLBACK_FINAL_STATUS" == "ok" ]]; then
    ...
    ui_warn "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前虽然复用的是既有 apply-plan / generated-output 边界，但默认仍先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发或真实 apply 的续跑入口。"
    if [[ -n "$RESUME_SOURCE_ACME_NEXT_STEP" ]]; then
      ui_warn "ACME 建议：$RESUME_SOURCE_ACME_NEXT_STEP"
    fi
  fi
