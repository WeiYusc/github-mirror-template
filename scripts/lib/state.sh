...
resume_strategy_prefers_review_boundary() {
  case "${1:-}" in
    post-rollback-inspection|post-repair-verification|repair-review-first|inspect-after-apply-attention|inspect-after-acme-placeholder|inspect-after-acme-real-execute-attempt)
      return 0
      ;;
    *)
      return 1
...
resume_strategy_review_boundary_family_text() {
  printf '%s\n' "inspect-after-apply-attention / inspect-after-acme-placeholder / inspect-after-acme-real-execute-attempt / repair-review-first / post-repair-verification / post-rollback-inspection"
}

resume_strategy_review_warning_message() {
  case "${1:-}" in
    inspect-after-acme-placeholder)
      printf '%s\n' "源运行的 ACME companion result 仍是 execute-placeholder / blocked 保守边界；当前会进入 inspect-after-acme-placeholder / review-first 续接，不会继承真实签发 / 证书落盘 / nginx 部署执行意图。"
      ;;
    inspect-after-acme-real-execute-attempt)
      printf '%s\n' "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前会进入 inspect-after-acme-real-execute-attempt / review-first 续接，应先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发 / 真实 apply 的续跑入口。"
      ;;
    inspect-after-apply-attention)
      printf '%s\n' "源运行的 apply 结果标记为需人工处理；当前会进入 inspect-after-apply-attention / review-first 续接，默认不会继承上次的真实 apply / nginx test 执行意图。"
      ;;
    *)
      return 1
      ;;
...
resume_source_acme_real_execute_reuse_warning_message() {
  local mode="${1:-resume}"
  if [[ "$mode" == "execute-refusal" ]]; then
    printf '%s\n' "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前虽然复用的是既有 apply-plan / generated-output 边界，但仍应先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发 / 真实 apply 的续跑入口。"
  else
    printf '%s\n' "源运行已带有 non-placeholder ACME real-execute-attempt companion result；当前虽然复用的是既有 apply-plan / generated-output 边界，但默认仍先复核 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发或真实 apply 的续跑入口。"
  fi
}

resume_strategy_execute_apply_refusal_message() {
  local resume_strategy="${1:-}"
  printf '当前 resume 策略 %s 不允许直接执行真实 apply；这类 inspection-first 续接（包括 %s）必须先按 doctor / repair / rollback 结论完成复查。\n' \
    "$resume_strategy" \
    "$(resume_strategy_review_boundary_family_text)"
}
...
state_plan_resume_runtime() {
  ...
  elif [[ "${RESUME_SOURCE_ACME_REAL_EXECUTE_ATTEMPT:-0}" == "1" ]]; then
    RESUME_STRATEGY="inspect-after-acme-real-execute-attempt"
    RESUME_STRATEGY_REASON="source ACME companion result already reached non-placeholder real execute attempt"
...
def derive_effective_resume_strategy(lineage: dict, apply_result: dict | None, repair_result: dict | None, rollback_result: dict | None, acme_issuance_result: dict | None = None):
    ...
    def strategy_phase(strategy: str) -> int:
        ...
        if strategy == "inspect-after-acme-placeholder":
            return 1
        if strategy == "inspect-after-acme-real-execute-attempt":
            return 1
    ...
    if rollback_result.get("mode", "") == "execute" and rollback_result.get("final_status", "") == "ok":
        ...
        direct_strategy = "inspect-after-acme-real-execute-attempt"
        direct_reason = "source ACME companion result already reached non-placeholder real execute attempt"
...
def choose_resume_strategy_priority_artifact(state: dict, lineage: dict):
    ...
    if resume_strategy == "inspect-after-acme-real-execute-attempt":
        path = first_existing_artifact(artifacts, "acme_issuance_result_json", "acme_issuance_result", "issue_result_json", "issue_result")
        if path:
            return ("acme-issuance-result", path, "当前 run 的 ACME companion result 已进入 non-placeholder real execute attempt；应先看 ACME / ISSUE result 与 operator prerequisites，不把 resume 当成真实签发或真实 apply 的续跑入口。")
...
def describe_resume_strategy_metadata(resume_strategy: str):
    resume_strategy = (resume_strategy or "").strip()
    metadata = {
        "focus": "",
    ...
    if resume_strategy == "inspect-after-acme-placeholder":
        metadata.update({
            ...
            "allowed": "复用输入/产物；显式 --run-apply-dry-run；人工复查 ACME companion result / issue result / operator prerequisites。",
            "disallowed": "不要把 --resume 当成真实 ACME issue / 真实 apply 续跑；不要直接带 --execute-apply。",
            "review_boundary": True,
        })
    elif resume_strategy == "inspect-after-acme-real-execute-attempt":
        metadata.update({
            "focus": "acme",
            "operator_hint": "优先核对 ACME / ISSUE result 与 operator prerequisites，确认当前是 non-placeholder real execute attempt 的复查边界，而不是把 resume 当成真实签发或真实 apply 的续跑入口。",
            "allowed": "复用输入/产物；显式 --run-apply-dry-run；人工复查 ACME / ISSUE result 与 operator prerequisites。",
            "disallowed": "不要把 --resume 当成真实签发或真实 apply 的续跑入口；不要直接带 --execute-apply。",
            "review_boundary": True,
        })
        ...
        metadata.update({
            ...
            "allowed": "复用输入/产物；显式 --run-apply-dry-run；先人工复查 apply result / recovery。",
            "disallowed": "不要把 --resume 当成真实 apply 重放；不要直接带 --execute-apply。",
            "review_boundary": True,
        })
    elif resume_strategy == "repair-review-first":
        metadata.update({
            "focus": "repair",
            "operator_hint": "优先查看 repair 结果与诊断摘要，确认应该 rollback 还是继续人工修复。",
            "allowed": "继续人工排查；必要时再次 repair；显式 dry-run。",
            "disallowed": "不要把未收口 repair run 当成普通 resume；不要跳过复查直接真实 apply。",
            "review_boundary": True,
        })
        ...
        metadata.update({
            ...
            "allowed": "复用可用产物；显式 dry-run；人工确认 nginx -t / 现场后再决定下一步。",
            "disallowed": "不要因为 repair passed 就默认继续 execute；不要直接带 --execute-apply。",
            "review_boundary": True,
        })
        ...
        metadata.update({
            ...
            "allowed": "复核 rollback 结果；显式 dry-run；必要时新开 run。",
            "disallowed": "不要把 rollback 后现场直接视为可继续 execute 的干净断点；不要直接带 --execute-apply。",
            "review_boundary": True,
        })
    elif resume_strategy in {"reuse-apply-plan", "reuse-generated-output", "reuse-preflight"}:
        metadata["operator_hint"] = "当前更像是复用既有产物继续推进；先确认复用产物仍然有效，再决定是否进入下一阶段。"
    elif resume_strategy == "re-enter-from-inputs":
        metadata["operator_hint"] = "当前只能从已保存输入重新进入；先确认输入仍然适用，再继续跑后续阶段。"
...
def choose_resume_strategy_suggestion_focus(lineage: dict):
    lineage = ensure_dict(lineage)
    resume_strategy = lineage.get("resume_strategy", "") or ""
    return describe_resume_strategy_metadata(resume_strategy).get("focus", "")


def describe_resume_strategy_action_boundary(resume_strategy: str):
    metadata = describe_resume_strategy_metadata(resume_strategy)
    allowed = metadata.get("allowed", "")
    disallowed = metadata.get("disallowed", "")
...
def resume_strategy_prefers_review_boundary_py(resume_strategy: str) -> bool:
    return bool(describe_resume_strategy_metadata(resume_strategy).get("review_boundary", False))
...
def print_resume_lineage_summary(state: dict, lineage: dict, lineage_chain):
    ...
    strategy_metadata = describe_resume_strategy_metadata(resume_strategy)
    print(f"- 操作建议：{strategy_metadata.get('operator_hint') or '先结合 state / result artifacts 做常规复核。'}")

    action_boundary = describe_resume_strategy_action_boundary(resume_strategy)
    if action_boundary is not None:
        print(f"- 允许动作：{action_boundary['allowed']}")
        print(f"- 禁止动作：{action_boundary['disallowed']}")
...
def print_resume_strategy_preview(state: dict, lineage: dict):
    state = ensure_dict(state)
    lineage = ensure_dict(lineage)
    resume_strategy = lineage.get("resume_strategy", "") or ""
    resume_strategy_reason = lineage.get("resume_strategy_reason", "") or ""
    strategy_metadata = describe_resume_strategy_metadata(resume_strategy)
    if not resume_strategy or not strategy_metadata.get("review_boundary", False):
        return

    print("[doctor] resume 预览")
    print(f"- 如果现在继续 --resume，预计策略：{resume_strategy}")
    if resume_strategy_reason:
        print(f"- 触发原因：{resume_strategy_reason}")
    ...
    preferred_priority_artifact = choose_resume_strategy_priority_artifact(state, lineage)
    if preferred_priority_artifact is not None:
        print_priority_artifact_hint("预计优先产物", preferred_priority_artifact)
    print()
...
def print_acme_issuance_result_summary(acme_issuance_result: dict, acme_issuance_result_json_path: str):
    ...
    if execution:
        print(f"- execution.challenge_strategy: {execution.get('challenge_strategy', '')}")
        print(f"- execution.client_adapter: {execution.get('client_adapter', '')}")
        print(f"- execution.materialization: {execution.get('materialization', '')}")
        print(f"- execution.fulfilled_challenge_strategy: {execution.get('fulfilled_challenge_strategy', '')}")
    ...
    artifacts = ensure_dict(acme_issuance_result.get("artifacts"))
    if artifacts:
        print(f"- artifacts.materialization: {artifacts.get('materialization', '')}")
...
if not is_effectively_resumed_run(state, raw_lineage):
    print_resume_strategy_preview(state, effective_lineage)
...
if suggestion is None:
    if resume_strategy_prefers_review_boundary_py(resume_strategy):
        suggestion = (
