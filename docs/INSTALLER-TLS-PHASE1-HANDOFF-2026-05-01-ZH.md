...
- 当前 `--execute` 占位路径还会额外把这个 future result 容器先落成 placeholder skeleton，明确记录“进入 execute result contract，但还没有真实 client execute / challenge fulfillment / 证书产物”
- 未来真实签发结果仍不得继续复用 `ISSUE-RESULT.{md,json}`，避免 operator / automation / resume logic 把计划结果与执行结果混成双义 artifact
...
5. 在动真实 execute 代码前，先按最小 skeleton 收口 future result：
   - `schema_kind/schema_version`
   - `mode/final_status`
   - `context`
   - `request`
   - `execution.attempted_hosts / fulfilled_challenge_strategy / client_invoked / issued_certificate`
   - `artifacts.cert_path / key_path / fullchain_path`
   - `deployment_boundary.*`
   - `recovery.recoverable / blocker_summary`
   - `next_step`
   - 并继续避免把这些真实 execute 字段回填进 `ISSUE-RESULT`
   - 当前可先维持 execute placeholder 语义：`final_status=blocked`、`client_invoked=false`、`issued_certificate=false`
