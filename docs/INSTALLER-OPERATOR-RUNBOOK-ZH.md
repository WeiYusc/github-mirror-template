...
- inspection-first / review-first 续接不再只有原来的 4 类；现在还多了两类 ACME 相关策略：
...
- 对 non-placeholder ACME real-execute-attempt：
  - resume 现在会直接进入 **`inspect-after-acme-real-execute-attempt`** 语义
  - 会优先复核 `ACME-ISSUANCE-RESULT.json` / `ISSUE-RESULT.json` / operator prerequisites
  - 不把这类 resume 当成真实签发或真实 apply 的续跑入口
  - 显式 `--execute-apply` 同样会被直接拒绝

### inspection-first 六类策略的处理矩阵

| strategy | 典型来源 | 默认先看什么 | 允许什么 | 不允许什么 |
| --- | --- | --- | --- | --- |
| `inspect-after-acme-placeholder` | `ACME-ISSUANCE-RESULT.json` 通过显式 placeholder marker 仍是 execute-placeholder / blocked，且未发生真实签发 / 证书落盘 / 部署执行 | `ACME-ISSUANCE-RESULT.json`、`ISSUE-RESULT.json`、`--doctor` 摘要 | 复用输入/产物；显式 `--run-apply-dry-run`；人工复查 ACME companion result 与后续前提 | 把 `--resume` 当成真实 ACME issue / 真实 apply 续跑；显式 `--execute-apply` |
| `inspect-after-acme-real-execute-attempt` | `ACME-ISSUANCE-RESULT.json` 已进入 non-placeholder real execute attempt，且仍需 operator 先复核 companion result / prerequisites | `ACME-ISSUANCE-RESULT.json`、`ISSUE-RESULT.json`、`--doctor` 摘要 | 复用输入/产物；显式 `--run-apply-dry-run`；人工复查 ACME companion result 与前提条件 | 把 `--resume` 当成真实 ACME issue / 真实 apply 续跑；显式 `--execute-apply` |
...
- 当前还新增了独立 `./acme-execute-http01.sh --issue-result-json <ISSUE-RESULT.json>`：
  - 它会独立落出 `ACME-ISSUANCE-RESULT.{md,json}`
  - 当前 cut 1 明确是 **non-placeholder / pre-client blocked**，用于表达“独立 execute 入口已存在，但真实 ACME runtime 仍未接通”
  - 它当前会保持：
    ...
    - `execution.issued_certificate=false`
    - `deployment_boundary.*=false`
- 这份独立 execute helper 结果同样**不是**真实签发成功/失败记录，只是把 execute companion result 从 placeholder 容器再往前推进一刀
- 这份 future execute companion result 现在还额外有一个**最小 skeleton 红线**：
  ...
  - 不因为看见 `--execute` 或独立 execute helper 已存在，就默认等于“证书已签出来 / 已可部署”
- 不安装 acme client
- 不真实签发证书
- 不改 live nginx
- 不 reload nginx
...
所以它当前解决的是“把 issue helper 的输入/检查/边界说清楚”，以及“把独立 execute companion contract 落成可消费入口”，不是“把 ACME lifecycle 做完”。
...
- 把“独立 execute helper 已存在”误判成“真实 client 已调用”
