...
当 `resume` / `doctor` 面对 inspection-first 六类策略时，当前更可靠的消费顺序不是“只看某一个 final/status 字段”，而是：
...
1. 先看 **当前可解析到的 companion/apply 结果** 是否已经足以重新推导 effective strategy
   ...
   - `ACME-ISSUANCE-RESULT.json` 已进入 non-placeholder real execute attempt（如 `intent.result_role=real-execute-attempt`、`intent.real_execution_performed=true`、或 `execution.client_invoked=true`，且 `placeholder.is_placeholder=false`）→ `inspect-after-acme-real-execute-attempt`
...
- `acme-execute-http01.sh`：独立 execute helper cut 1 产出的 non-placeholder / pre-client blocked companion result

### 7.6 `ACME-ISSUANCE-RESULT.json` 最小 contract skeleton（当前为 execute placeholder，future real execute 继续沿用）

> 这一节定义的是 **future real execute 仍可沿用的最小稳定骨架**，也是当前 `acme-issue-http01.sh --execute` / `acme-execute-http01.sh --issue-result-json <ISSUE-RESULT.json>` 已开始落盘的 execute companion result 下限。
...
> 其中 `acme-issue-http01.sh --execute` 产出的是 placeholder skeleton，而 `acme-execute-http01.sh` 产出的是 non-placeholder / pre-client blocked companion result。
...
- `execution.challenge_strategy`：把 runtime 视角下实际选择的 challenge strategy 单独钉成稳定字段，避免未来只能从 request/pending plan 反推
- `execution.client_adapter`：把 runtime 视角下准备使用或已进入的 client adapter 单独钉成稳定字段，避免 future real execute 再混回 request/pending plan
- `execution.materialization`：把 runtime 侧对证书产物落盘阶段的最小状态单独钉住，避免只剩中文 blocker 或 path 为空时难以分辨
...
- `artifacts.materialization`：把 artifact 侧当前是否仍未落盘、是否只是保留指针位的状态显式化，避免 future real execute 时 path 空值语义过载
...
```json
{
  ...
  "execution": {
    ...
    "challenge_strategy": "standalone",
    "client_adapter": "acme.sh",
...
5. 若 `ACME-ISSUANCE-RESULT.json` 已进入 **non-placeholder real-execute-attempt**（例如 `intent.result_role=real-execute-attempt`、`intent.real_execution_performed=true`、或 `execution.client_invoked=true`，且未标记 placeholder），当前也应保持 review-first 恢复边界：
   - planner / doctor 应直接落到 `inspect-after-acme-real-execute-attempt`
...
对 inspection-first 六类策略，当前尤其不要把下面这些内容当成唯一真相源：
