...
2. 若源 run 的 `ACME-ISSUANCE-RESULT.json` 已进入 non-placeholder real execute attempt → `inspect-after-acme-real-execute-attempt`
3. 若源 run 已执行 rollback 且成功 → `post-rollback-inspection`
4. 若源 run 的 repair 已重跑 `nginx -t` 且通过 → `post-repair-verification`
5. 若源 run repair 仍是 `needs-attention` / `blocked` → `repair-review-first`
6. 若源 run apply 明确 `resume_recommended != 1` → `inspect-after-apply-attention`
7. 若可复用 apply plan → `reuse-apply-plan`
8. 若可复用 generator 输出 → `reuse-generated-output`
9. 若可复用 preflight/config → `reuse-preflight`
10. 否则 → `re-enter-from-inputs`

这也是 `lineage.resume_strategy` 和 `lineage.resume_strategy_reason` 的来源。

但这里不再需要额外记住一层“effective review-first override”了：

- `lineage.resume_strategy` 现在直接记录当前命中的**实际策略槽位**
- 如果源 run 带有 **non-placeholder ACME real-execute-attempt companion result**（例如 `intent.result_role=real-execute-attempt`、`intent.real_execution_performed=true`、或 `execution.client_invoked=true`，且 `placeholder.is_placeholder=false`），当前实现会直接落到 `inspect-after-acme-real-execute-attempt`
- 这类 resume 可以继续复用既有 `apply-plan / generated-output` 边界，却**不能**被当成真实签发或真实 apply 的续跑入口
- `RESUME_SOURCE_ACME_REAL_EXECUTE_ATTEMPT=1` 仍可保留作为旧 run / 兼容路径的保护信号，但不再是主叙事

因此读 `resume_strategy` 时，优先把 strategy 本身当作主语义来源；兼容 flag 只用于兜底旧状态。

---

### 8.4 当前哪些 strategy 禁止直接 execute

`resume_strategy_allows_execute()` 当前对这 6 类 inspection-first strategy 明确返回“不允许直接进入真实 apply”：
...
`resume_allows_execute()` 仍保留一层兼容兜底：

- 对旧状态/旧 run，即使名义 strategy 没写出新值
- 只要 `RESUME_SOURCE_ACME_REAL_EXECUTE_ATTEMPT=1`
- 当前也会继续拒绝显式 `--execute-apply`

如果在这些 review-first 语义下显式传：
...
但在这些 inspection-first / review-first 续接下，当前仍允许：
...
| `inspect-after-acme-real-execute-attempt` | `ACME-ISSUANCE-RESULT.json` 已进入 non-placeholder real execute attempt（如 `intent.result_role=real-execute-attempt`、`intent.real_execution_performed=true`、`execution.client_invoked=true`，且 `placeholder.is_placeholder=false`） | 进入 ACME real-execute-attempt 复查；优先看 ACME / ISSUE result 与 operator prerequisites，不把 resume 当真实签发或真实 apply 续跑 | 允许 | 不允许 | `inputs.env`、`deploy.generated.yaml`、`preflight.generated.*`、`INSTALLER-SUMMARY.generated.json` | `ACME-ISSUANCE-RESULT.json`、`ISSUE-RESULT.json`、必要时 `APPLY-RESULT.json` / `REPAIR-RESULT.json` / `ROLLBACK-RESULT.json` |
...
| `reuse-apply-plan` | 源 run 的 `APPLY-PLAN.json` 可复用 | 直接从 apply-plan 边界续接；一般可继续 dry-run/execute 决策 | 允许 | 一般允许 | `inputs.env`、`deploy.generated.yaml`、`preflight.generated.*`、`INSTALLER-SUMMARY.generated.json` | 已存在的 `APPLY-PLAN.json`、共享 `output_dir`、后续新产出的 `APPLY-RESULT.json` |
| `reuse-generated-output` | 源 run 的 generated output 目录可复用 | 跳过 generator，重新生成/确认 apply plan 后继续 | 允许 | 一般允许 | `inputs.env`、`deploy.generated.yaml`、`preflight.generated.*`、`INSTALLER-SUMMARY.generated.json` | 已存在 `output_dir_abs` 与其中渲染结果 |
| `reuse-preflight` | 源 run 的 config + preflight 可复用 | 跳过输入/preflight，从 generator 之后继续 | 允许 | 一般允许 | `inputs.env`、`deploy.generated.yaml`、`preflight.generated.*`、`INSTALLER-SUMMARY.generated.json` | 源 run 的可复用 config/preflight 上下文 |
...
可以把 inspection-first 那 6 类再统一记成：
...
- `docs/INSTALLER-RESULT-CONTRACTS-ZH.md`：当前结果契约入口，涵盖 `state.json`、`INSTALLER-SUMMARY.json`、`APPLY-PLAN.json`、`APPLY-RESULT.json`、`REPAIR-RESULT.json`、`ROLLBACK-RESULT.json`、`ISSUE-RESULT.json` 与 `ACME-ISSUANCE-RESULT.json` 的职责边界、稳定字段、消费顺序与兼容策略
- `docs/archive/INSTALLER-TLS-STABLE-STOP-2026-05-03-ZH.md`：TLS Phase 1 / 2 / 3 以及 review-first 修正后的最新稳定停点归档
