...
- inspection-first resume 语义（`inspect-after-apply-attention` / `inspect-after-acme-placeholder` / `inspect-after-acme-real-execute-attempt` / `repair-review-first` / `post-repair-verification` / `post-rollback-inspection`）
...
- `./acme-execute-http01.sh --issue-result-json <ISSUE-RESULT.json>`：当前是**独立 execute helper cut 1**
  - 只消费既有 `ISSUE-RESULT.json`
  - 独立落盘 `ACME-ISSUANCE-RESULT.{md,json}`
  - 当前明确是 **non-placeholder / pre-client blocked** companion result：`intent.result_role=real-execute-attempt`，但 `intent.real_execution_performed=false`、`execution.client_invoked=false`、`issued_certificate=false`
  - 这样做是为了把“独立 execute 入口已存在”与“真实 ACME client/runtime 仍未接通”分开表达
- 目前已在 `docs/INSTALLER-RESULT-CONTRACTS-ZH.md` 中把这个 future companion result 的**最小 skeleton**钉成文档契约，并且当前 `--execute` 占位路径会额外落一个同骨架的 placeholder 文件：
  ...
  - 当前 placeholder 仍明确保持：`intent.real_execution_performed=false`、`client_invoked=false`、`issued_certificate=false`、deployment boundary 全 false
  - 当前独立 execute helper cut 1 虽已是 **non-placeholder**，但仍明确保持 pre-client blocked：不会调用 ACME client、不会写证书、不会跨入 live deploy boundary
- 这样分叉是为了避免 operator / automation / resume logic 混淆：
  - 当前 helper 的前置检查/计划结果
  - `mode=execute` 的占位语义
  - 已独立落盘但仍未真实调用 client 的 execute companion result
...
- 关键参数至少包括：
  ...
  - `./acme-execute-http01.sh`
    ...
    - `--result-file <path>`
- `acme-issue-http01.sh` 会输出 `ISSUE-RESULT.md` / `ISSUE-RESULT.json`
- 当显式带 `--execute` 时，`acme-issue-http01.sh` 还会额外输出 `ACME-ISSUANCE-RESULT.md` / `ACME-ISSUANCE-RESULT.json` 的 placeholder skeleton
- `acme-execute-http01.sh` 会独立输出 `ACME-ISSUANCE-RESULT.md` / `ACME-ISSUANCE-RESULT.json` 的 non-placeholder pre-client blocked companion result
- 当前 ACME execute companion contract 已显式保留三类 runtime boundary 槽位：
  - `execution.challenge_strategy`
  - `execution.client_adapter`
  - `execution.materialization` / `artifacts.materialization`
- 若来源 run 可定位到 `state.json`，helper 会把 `issue_result` / `issue_result_json` / `acme_issuance_result` / `acme_issuance_result_json` 回写到该 run 的 `artifacts`，并同步到 generated summary / output summary / `journal.jsonl`
- 当前 **不会** 真实签发证书、不会安装 acme client、不会改 live nginx、不会 reload nginx、不会写入证书文件
- 所以这两层 helper 当前的定位仍是 **planning / evidence + conservative execute contract helper**，不是完整 ACME lifecycle
...
- 同时如果该 run 已经有 `REPAIR-RESULT.json` / `ROLLBACK-RESULT.json` / `ACME-ISSUANCE-RESULT.json`：
  ...
  - ACME companion result 已进入 non-placeholder real execute attempt 时，resume 会优先进入 **`inspect-after-acme-real-execute-attempt`** 语义
