...
- 目前常见的 inspection-first 语义包括：`inspect-after-apply-attention`、`inspect-after-acme-placeholder`、`inspect-after-acme-real-execute-attempt`、`post-repair-verification`、`repair-review-first`、`post-rollback-inspection`
...
| `inspect-after-acme-placeholder` | `ACME-ISSUANCE-RESULT.json` 仍停在 execute-placeholder / blocked 保守边界，且未发生真实签发 / 证书落盘 / 部署执行 | 进入 ACME placeholder 复查；先看 `ACME-ISSUANCE-RESULT.json` / `ISSUE-RESULT.json` / `--doctor`，不把 `--resume` 当真实签发或真实 apply 续跑 | 允许 | 不允许 | `inputs.env`、run-local `deploy.generated.yaml`、run-local `preflight.generated.*`、run-local `INSTALLER-SUMMARY.generated.json` | `ACME-ISSUANCE-RESULT.json`、`ISSUE-RESULT.json`、必要时 `APPLY-RESULT.json` / `REPAIR-RESULT.json` / `ROLLBACK-RESULT.json` |
| `inspect-after-acme-real-execute-attempt` | `ACME-ISSUANCE-RESULT.json` 已进入 non-placeholder real execute attempt，且仍需 operator 先复核 companion result / prerequisites | 进入 ACME real-execute-attempt 复查；先看 `ACME-ISSUANCE-RESULT.json` / `ISSUE-RESULT.json` / `--doctor`，不把 `--resume` 当真实签发或真实 apply 续跑 | 允许 | 不允许 | `inputs.env`、run-local `deploy.generated.yaml`、run-local `preflight.generated.*`、run-local `INSTALLER-SUMMARY.generated.json` | `ACME-ISSUANCE-RESULT.json`、`ISSUE-RESULT.json`、必要时 `APPLY-RESULT.json` / `REPAIR-RESULT.json` / `ROLLBACK-RESULT.json` |
...
可以把 inspection-first 那 6 类再记成一句话：
...
- inspection-first resume 语义（包括 `inspect-after-apply-attention` / `inspect-after-acme-placeholder` / `inspect-after-acme-real-execute-attempt` / `repair-review-first` / `post-rollback-inspection`）
